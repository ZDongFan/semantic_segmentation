...
这是一个面向 QGIS 3.28 LTR 的地物分类插件，可对当前活动栅格图层或磁盘影像执行 PaddleRS 语义分割推理，并把结果作为新的栅格图层加载回 QGIS。
...
- 普通 JPG/PNG/TIF 先 resize 到 `512 x 512`，再执行一次 `predict()`。
...
└─ land_cover_classification/          # QGIS 插件主体目录
   ...
   │                                   # Qt Designer 设计的界面文件
   ├─ inference.py                     # 核心推理逻辑，包含 QgsTask 和结果写出
