...
这是一个面向 QGIS 3.28 LTR 的地物分类插件，可对当前活动栅格图层或本地影像执行基于 PaddleRS 的遥感语义分割，并将结果组织为“类别栅格 -> 草稿矢量 -> 最终成果”的人工确认流程。推理生成草稿层后，插件还支持基于 SAM1 ViT-B 的 AI 辅助编辑，用正负点提示生成 mask 预览，并将结果追加或替换到现有草稿图层。
...
- 仓库约定了默认 SAM ViT-B 权重位置与离线 wheelhouse 目录，用户可按文档准备文件后在目标机创建插件专用 SAM 虚拟环境。
...
AI 辅助编辑控件位于 `编辑与导出` 页签中。启动后，插件会加载 `land_cover_classification/models/sam/sam_vit_b_01ec64.pth`，并使用 `land_cover_classification/vendor/sam_runtime/venv/` 中的独立 Python 环境运行 `sam_worker.py`。QGIS 主进程只负责界面、坐标转换、预览显示和草稿层写回，不在主进程中导入 `torch` 或 `segment_anything`。预览不会写入真实草稿层，也不会创建单独的 AI 图层；只有用户点击追加或替换时，AI 结果才提交到草稿层。为适配当前滑坡识别任务，一次 AI 编辑只生成一个滑坡草稿对象：SAM 子进程提取最大外轮廓，QGIS 侧也会兜底选择最大单面几何，并用稳定的单面 RubberBand 显示预览。
...
5. 如需使用 AI 辅助编辑，请按文档准备 `land_cover_classification/models/sam/sam_vit_b_01ec64.pth` 与 `land_cover_classification/vendor/sam_runtime/wheels/` 下所需文件，再在目标机运行 `land_cover_classification/vendor/sam_runtime/create_sam_venv.bat` 或 `create_sam_venv.sh` 创建 SAM 专用虚拟环境。
...
5. 推理完成后插件自动切到 `编辑与导出` 页签。可直接手动编辑草稿层，也可以点击“启动 AI 编辑”后在地图画布上左键添加正样本点、右键添加负样本点，预览满意后追加或替换草稿对象。连续添加点时界面只以最新点集刷新预览，避免多轮 SAM 预测同时回写画布。
...
`-- land_cover_classification/         # QGIS 插件主体目录
    ...
    |   `-- sam_runtime/               # SAM 专用 venv 创建脚本与离线 wheels
    `-- models/                        # 模型根目录
        |-- semantic_segmentation/     # 语义分割模型默认存放位置
        `-- sam/                       # SAM ViT-B 权重默认存放位置
```

## 许可

本项目以 Apache License 2.0 发布，详见 [`LICENSE`](LICENSE)。内置的 PaddleRS 代码同样遵循 Apache-2.0，相关许可位于 `land_cover_classification/vendor/PaddleRS/LICENSE`。SAM、PyTorch、segment-anything 以及离线 wheels 的第三方说明位于 `land_cover_classification/vendor/sam_runtime/NOTICE.txt`。
